...
void GameStaticData::SaveDay()
{
	...
	fopen_s(&pFile, "SaveData.txt", "wb"); // 파일 열어서 객체화된 파일에 가져다 놓기
	...
	intVector_.push_back(static_cast<int>(savedDay_)); // 버퍼에 현재 날짜 


	size_ = intVector_.size();
	//fwrite(&intVector_[0], sizeof(int), 1, pFile); // 벡터 배열에다가 해당 파일의 글자들을 모두 복사해 써붙인다.
...
void GameStaticData::LoadDay()
{
	...
	fread(&intVector_[0], sizeof(int), 1, pFile);
	fclose(pFile);
